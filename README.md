...
![Codex Pet Viewer 预览台](img/1.png)

### 嵌入 SDK 示例

把 `embed/codex-pet.js` 引入任意网页，用 `data-*` 属性或 JS API 挂载宠物。

![CodexPet 嵌入 SDK 示例](img/2.png)

### 博客实战嵌入

挂到业务站点后的悬浮效果（示例：[blog.pljzy.top](https://blog.pljzy.top/)）。

![博客页面中的桌面宠物](img/3.png)
...
也可以用 npx 临时托管（无需 `server.js`）：

```bash
npm run serve
# 等价于: npx --yes serve -l 8765
```
